...
```bash
flatpak install --user apps/frontend/dist/DEVAPEX-*.flatpak
```
...
```bash
# Make executable
chmod +x DEVAPEX-*-linux-x86_64.AppImage

# Run
./DEVAPEX-*-linux-x86_64.AppImage
```
...
```bash
sudo dpkg -i DEVAPEX-*-linux-amd64.deb
```
...
ldd ./DEVAPEX-*-linux-x86_64.AppImage

# Try running with debug output
./DEVAPEX-*-linux-x86_64.AppImage --verbose
```
