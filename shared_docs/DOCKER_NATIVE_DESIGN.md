...
> Design document for converting DEVAPEX from Electron to a containerized web application.

## Executive Summary

This document outlines the architecture for a Docker-native version of DEVAPEX that:
...
| POST | `/api/projects/{id}/initialize` | Initialize DEVAPEX in project |
| GET | `/api/projects/{id}/version` | Check DEVAPEX version |
...
WORKDIR /app
COPY DEVAPEX/requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Install API dependencies
COPY DEVAPEX/api/requirements.txt ./api-requirements.txt
RUN pip install --no-cache-dir -r api-requirements.txt

# Copy application code
COPY DEVAPEX/ ./DEVAPEX/

# --- Frontend Build Stage ---
FROM node:22-alpine AS frontend-build

WORKDIR /app
COPY DEVAPEX-ui/package*.json ./
RUN npm ci
...
services:
  app:
    ...
    volumes:
      ...
      - DEVAPEX-claude:/home/claude/.claude
    environment:
  ...
  falkordb:
    ...
    container_name: DEVAPEX-falkordb
    volumes:
      - falkordb-data:/data
    networks:
      - DEVAPEX-net
  ...
  graphiti-mcp:
    ...
    container_name: DEVAPEX-graphiti
...
1. OAuth tokens stored in named volume (`DEVAPEX-claude`)
