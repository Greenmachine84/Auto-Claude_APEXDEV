...
class MemoryBridge:
    ...
    def __init__(self, config: Optional[BridgeConfig] = None):
        ...
        self._local_store: Optional[Any] = None
    ...
    def add_sync_callback(
        self,
        callback: Callable[[str, SyncResult], None]
    ) -> None:
    ...
    async def sync(
        ...
        for name, provider in providers.items():
            try:
                result = await self._sync_provider(
    ...
    async def start_background_sync(self) -> None:
        ...
        logger.info("Started background sync (interval=%ds)", 
    ...
    async def export_data(
        ...
        """
        episodes = await self._get_local_episodes(
            since=since,
            agent_ids=agent_ids
        )
    ...
    async def _sync_provider(
        ...
        if not await provider.connect():
            return SyncResult(
                status=SyncStatus.FAILED,
                errors=[f"Failed to connect to {name}"]
            )
    ...
    async def _get_local_episode(self, episode_id: str) -> Optional[EpisodeRecord]:
