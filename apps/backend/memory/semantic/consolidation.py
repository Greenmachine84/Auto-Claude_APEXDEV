...
from .knowledge_graph import KnowledgeGraph, KGNode, KGEdge, EdgeType
...
class KnowledgeConsolidator:
    ...
    def deduplicate_entry(self, entry: VectorEntry) -> Optional[str]:
        """Check if entry is duplicate, return existing ID if so."""
        similar = self._vector_store.search(entry.vector, k=5, namespace=entry.namespace)
        for result in similar:
            if result.score >= self._threshold and result.entry.id != entry.id:
                logger.info("Dedup: %s matches %s (score=%.3f)", entry.id, result.entry.id, result.score)
    ...
    def link_to_graph(self, entry: VectorEntry, node_type: str = "concept") -> str:
        ...
        node = KGNode(
            id=entry.id, node_type=node_type, name=entry.content[:100],
            properties=entry.metadata, embedding=entry.vector
        )
