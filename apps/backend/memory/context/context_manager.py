...
from .context_window import ContextWindow, ContextEntry, ContextRole, ContextPriority
...
class ContextManager:
    ...
    def __init__(
        ...
        self._window = ContextWindow(max_tokens=max_tokens, reserve_tokens=reserve_tokens)
    ...
    def add_system(self, content: str, token_count: int = 0) -> str:
        return self._add_entry(ContextRole.SYSTEM, content, token_count, ContextPriority.CRITICAL, pinned=True)
    
    def add_user(self, content: str, token_count: int = 0) -> str:
        return self._add_entry(ContextRole.USER, content, token_count, ContextPriority.HIGH)
    
    def add_assistant(self, content: str, token_count: int = 0) -> str:
        return self._add_entry(ContextRole.ASSISTANT, content, token_count, ContextPriority.NORMAL)
    
    def add_tool(self, content: str, token_count: int = 0) -> str:
        return self._add_entry(ContextRole.TOOL, content, token_count, ContextPriority.NORMAL)
    
    def _add_entry(self, role: ContextRole, content: str, token_count: int, priority: ContextPriority, pinned: bool = False) -> str:
        entry_id = str(uuid.uuid4())
        tokens = token_count or len(content.split()) * 2  # Rough estimate
        
        entry = ContextEntry(
            id=entry_id, role=role, content=content, priority=priority,
            token_count=tokens, pinned=pinned
        )
    ...
    def _should_compress(self) -> bool:
        usage = self._window.total_tokens() / (self._window.total_tokens() + self._window.available_tokens())
