...
class RelevanceScorer:
    ...
    def score(self, context_item: Any, task: Task) -> float:
        ...
        total = sum(factors.get(k, 0.0) * w for k, w in self._weights.items())
    ...
    def rank(self, items: list[Any], task: Task) -> list[ScoredItem]:
        ...
        for item in items:
            ...
            total = sum(factors.get(k, 0.0) * w for k, w in self._weights.items())
            scored.append(
                ScoredItem(item=item, score=min(1.0, max(0.0, total)), factors=factors)
            )
    ...
    def _compute_factors(self, item: Any, task: Task) -> dict[str, float]:
        ...
        if task.keywords:
            matches = sum(1 for kw in task.keywords if kw.lower() in item_text)
    ...
    def _extract_time(self, item: Any) -> datetime | None:
    ...
    def _extract_success(self, item: Any) -> bool | None:
    ...
    def _extract_agent(self, item: Any) -> str | None:
