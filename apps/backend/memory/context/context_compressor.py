...
class ContextCompressor:
    ...
    def compress(self, entries: List[ContextEntry], target_tokens: int) -> tuple[List[ContextEntry], CompressionResult]:
        ...
        return compressed, CompressionResult(
            ...
            entries_merged=merged, duration_ms=duration
        )
    
    def _compress_entries(self, entries: List[ContextEntry], target: int) -> tuple[List[ContextEntry], int]:
        pinned = [e for e in entries if e.pinned or e.role == ContextRole.SYSTEM]
        compressible = [e for e in entries if not e.pinned and e.role != ContextRole.SYSTEM]
        ...
        for role, group in groups:
            if len(group) > 1:
                ...
                import uuid
                compressed_groups.append(ContextEntry(
                    id=str(uuid.uuid4()), role=role, content=merged_content,
    ...
    def _group_by_role(self, entries: List[ContextEntry]) -> List[tuple[ContextRole, List[ContextEntry]]]:
        if not entries:
            return []
