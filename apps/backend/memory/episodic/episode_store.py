...
class EpisodeStore:
    ...
    def store(self, episode: EpisodeRecord) -> str:
        ...
        conn.execute(
            """INSERT OR REPLACE INTO episodes 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                episode.id, episode.agent_id, episode.agent_type, episode.task_id,
                episode.input_text, episode.output_text, episode.reasoning,
                episode.outcome.value, episode.severity.value,
                json.dumps(episode.tools_used),
                json.dumps([t.to_dict() for t in episode.tool_invocations]),
                episode.duration_ms, episode.created_at, episode.updated_at,
    ...
    def _row_to_episode(self, row: sqlite3.Row) -> EpisodeRecord:
        from ..types.episode_types import EpisodeMetadata, ToolInvocation, EpisodeSeverity
        return EpisodeRecord(
            id=row["id"], agent_id=row["agent_id"], agent_type=row["agent_type"] or "",
            task_id=row["task_id"], input_text=row["input_text"], output_text=row["output_text"],
            reasoning=row["reasoning"], outcome=EpisodeOutcome(row["outcome"]),
            severity=EpisodeSeverity(row["severity"]),
            tools_used=json.loads(row["tools_used"] or "[]"),
            tool_invocations=[ToolInvocation.from_dict(t) for t in json.loads(row["tool_invocations"] or "[]")],
