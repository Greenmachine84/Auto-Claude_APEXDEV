...
class RetentionPolicy:
    ...
    def _setup_default_rules(self) -> None:
        self._rules = [
            ...
            RetentionRule(
                "delete_old", "age", self._max_age, RetentionAction.DELETE, 10
            ),
            RetentionRule(
                "archive_stale", "age", self._max_age / 2, RetentionAction.ARCHIVE, 5
            ),
        ]
    ...
    def evaluate(
        self, episode: EpisodeRecord, agent_episode_count: int = 0
    ) -> RetentionAction:
        now = datetime.utcnow()
    ...
    def apply(
        self, episodes: list[EpisodeRecord], agent_counts: dict[str, int]
    ) -> RetentionResult:
