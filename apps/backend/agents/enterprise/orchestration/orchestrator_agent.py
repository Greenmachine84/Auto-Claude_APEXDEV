...
from .pipeline_manager import Pipeline, PipelineManager
...
class OrchestratorAgent(BaseEnterpriseAgent):
    ...
    DEFAULT_SYSTEM_PROMPT: ClassVar[
        str
    ] = """You are an expert system orchestrator with deep knowledge of:
...
Synthesize results into coherent outputs."""
    ...
    def get_agent(self, agent_id: str) -> BaseEnterpriseAgent | None:
    ...
    async def coordinate_agents(
        ...
        aggregated = self.aggregator.aggregate(
            [{"agent_id": aid, "result": r} for aid, r in zip(agent_ids, results)]
        )
