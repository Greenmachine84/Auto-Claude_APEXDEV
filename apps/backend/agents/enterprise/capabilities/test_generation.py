...
class TestGenerationCapability:
    ...
    def generate_edge_case_tests(
        ...
        """
        edge_cases = [
            ("empty_input", "should handle empty input", f"{function_name}('') is not None"),
            ("null_input", "should handle null/None input", f"{function_name}(None) is not None or True"),
            ("large_input", "should handle large input", f"len(str({function_name}('x' * 10000))) >= 0"),
        ]
    ...
    def calculate_coverage_estimate(
        ...
        tested_functions = len(set(
            t.name.replace("test_", "").split("_")[0]
