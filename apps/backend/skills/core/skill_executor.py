...
from skills.core.base_skill import SkillContext, SkillResult, SkillStatus
from skills.core.skill_registry import SkillNotFoundError, SkillRegistry
...
class SkillExecutor:
    ...
    async def execute_chain(
        ...
        for skill_name in skill_names:
            ...
            if not result.success:
                success = False
                if stop_on_failure:
                    logger.warning(f"Chain stopped due to failure in {skill_name}")
                    break
            else:
    ...
    async def execute_parallel(
        ...
        tasks = [self.execute(skill_name, context) for skill_name in skill_names]
    ...
    async def execute_plan(
        self, plan: ExecutionPlan, context: SkillContext
    ) -> ChainResult:
