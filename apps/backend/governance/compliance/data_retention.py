...
class DataRetentionManager:
    ...
    def register_deletion_callback(
        self, category: DataCategory, callback: Callable[[datetime], int]
    ) -> None:
        """
    ...
    def _archive_data(self, category: DataCategory, cutoff: datetime) -> None:
        ...
        archive_file = (
            self._archive_path / f"{category.value}_{cutoff.strftime('%Y%m%d')}.json"
        )
    ...
    def get_deletion_history(
        self, category: DataCategory | None = None, limit: int = 100
    ) -> list[DeletionRecord]:
