...
from ..models import ComplianceEventType
...
class ComplianceLogger:
    ...
    def log(
        ...
        log_method(f"[{event_type.value}] {provider}/{user_id}: {action} -> {result}")
    ...
    def log_quota_usage(
        ...
        """Log quota usage."""
        return self.log(
            event_type=ComplianceEventType.QUOTA_EXCEEDED
            if remaining <= 0
            else ComplianceEventType.POLICY_EVALUATION,
            provider=provider,
            user_id=user_id,
            action="quota_usage",
            result="recorded",
    ...
    def log_approval(
        ...
        """Log approval event."""
        return self.log(
            event_type=ComplianceEventType.APPROVAL_GRANTED
            if status == "approved"
            else ComplianceEventType.APPROVAL_DENIED,
            provider=provider,
            user_id=user_id,
    ...
    def log_access(
        ...
        return self.log(
            ...
            if granted
            else ComplianceEventType.ACCESS_DENIED,
            provider=provider,
            user_id=user_id,
    ...
    def add_handler(self, handler: Callable[[ComplianceLogEntry], None]) -> None:
        """Add log handler."""
