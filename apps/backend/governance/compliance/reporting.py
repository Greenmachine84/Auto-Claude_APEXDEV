...
from .compliance_logger import ComplianceLogEntry, ComplianceLogger
...
class ComplianceReporter:
    ...
    def __init__(
        self,
        compliance_logger: ComplianceLogger | None = None,
    ...
    def _get_period_start(self, period: ReportPeriod, end_date: datetime) -> datetime:
    ...
    def _generate_policy_section(
        self, entries: list[ComplianceLogEntry]
    ) -> ReportSection:
        """Generate policy evaluation section."""
        policy_entries = [e for e in entries if "policy" in e.action.lower()]
    ...
    def _generate_access_section(
        ...
        access_entries = [e for e in entries if "access" in e.action.lower()]
    ...
    def _generate_rate_limit_section(
        ...
        rate_entries = [e for e in entries if "rate" in e.action.lower()]
    ...
    def _generate_approval_section(
        ...
        approval_entries = [e for e in entries if "approval" in e.action.lower()]
    ...
    def _generate_audit_section(self, entries: list[AuditEntry]) -> ReportSection:
    ...
    def _calculate_compliance_score(self, entries: list[ComplianceLogEntry]) -> float:
        """Calculate overall compliance score (0-100)."""
        if not entries:
            return 100.0

        violations = sum(
            1 for e in entries if e.result in ("denied", "limited", "rejected")
        )

        return max(0, 100 - (violations / len(entries) * 100))

    def _calculate_risk_score(
        self, log_entries: list[ComplianceLogEntry], audit_entries: list[AuditEntry]
    ) -> float:
        """Calculate risk score (0-100, lower is better)."""
        risk = 0.0

        # Policy violations increase risk
        violations = sum(1 for e in log_entries if e.result == "denied")
        risk += min(violations * 2, 30)

        # Configuration changes increase risk
        config_changes = sum(1 for e in audit_entries if "config" in e.action)
    ...
    def _generate_executive_summary(
        self, sections: list[ReportSection], compliance_score: float, risk_score: float
    ) -> str:
        """Generate executive summary."""
        summary_parts = [
