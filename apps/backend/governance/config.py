...
PROVIDER_RATE_LIMITS: dict[str, dict[str, int | None]] = {
    "copilot": {
...
PROVIDER_QUOTAS: dict[str, dict[str, float | None]] = {
    "copilot": {
...
class GovernanceSettings:
    ...
    def from_env(cls) -> "GovernanceSettings":
        ...
        return cls(
            ...
            == "true",
            rate_limit_storage=os.getenv("GOVERNANCE_RATE_LIMIT_STORAGE", "memory"),
            quota_enforcement_enabled=os.getenv(
                "GOVERNANCE_QUOTA_ENABLED", "true"
            ).lower()
            == "true",
            quota_warning_threshold_percent=float(
                os.getenv("GOVERNANCE_QUOTA_WARNING_THRESHOLD", "80.0")
            ),
            ...
            approval_default_timeout_hours=int(
                os.getenv("GOVERNANCE_APPROVAL_TIMEOUT_HOURS", "24")
            ),
            ...
            compliance_log_retention_days=int(
                os.getenv("GOVERNANCE_LOG_RETENTION_DAYS", "90")
            ),
        )
...
def get_provider_rate_limit(
    provider: str, limit_type: str = "requests_per_minute"
) -> int | None:
...
def get_provider_quota(
    provider: str, quota_type: str = "monthly_cost_usd"
) -> float | None:
