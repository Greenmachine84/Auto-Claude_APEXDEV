...
from ..models import SUPPORTED_PROVIDERS, QuotaUsage
...
class InMemoryStorage:
    ...
    def cleanup_expired(self) -> int:
        ...
        with self._lock:
            now = datetime.utcnow()
            expired = [
                k for k, (_, expiry) in self._data.items() if expiry and now > expiry
            ]
            for key in expired:
                del self._data[key]
                count += 1
...
class LimitStorage:
    ...
    def save_rate_limit_usage(
        self, provider: str, user_id: str, count: int, tokens: int, window_seconds: int
    ) -> bool:
    ...
    # Quota storage
    def save_quota_usage(self, provider: str, user_id: str, usage: QuotaUsage) -> bool:
        """Save quota usage."""
    ...
    def get_quota_usage(self, provider: str, user_id: str) -> QuotaUsage | None:
        """Get quota usage."""
        key = self._make_key("quota", provider, user_id)
        data = self._backend.get(key)
        if not data:
    ...
    def save_throttle_state(self, provider: str, state: str, backoff: float) -> bool:
        """Save throttle state."""
    ...
    def get_throttle_state(self, provider: str) -> dict[str, Any] | None:
        """Get throttle state."""
        key = self._make_key("throttle", provider)
        data = self._backend.get(key)
        if data:
