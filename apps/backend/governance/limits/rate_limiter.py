...
from ..models import SUPPORTED_PROVIDERS, RateLimitResult
...
class RateLimiter:
        ...
        for provider in SUPPORTED_PROVIDERS:
            ...
            if rpm:
                self._limits[provider] = ProviderRateLimit(
                    ...
                    tokens_per_minute=tpm // 1440
                    if tpm
                    else None,  # Convert daily to per-minute
                    window_seconds=60,
                )
        logger.info(
            f"Initialized rate limiter for {len(SUPPORTED_PROVIDERS)} providers"
        )

    def _get_window(
        self, provider: str, user_id: str, window_seconds: int = 60
    ) -> SlidingWindow:
    ...
    def check(self, provider: str, user_id: str, tokens: int = 0) -> RateLimitResult:
        """
    ...
    def record(self, provider: str, user_id: str, tokens: int = 0) -> None:
    ...
    def check_and_record(
        self, provider: str, user_id: str, tokens: int = 0
    ) -> RateLimitResult:
