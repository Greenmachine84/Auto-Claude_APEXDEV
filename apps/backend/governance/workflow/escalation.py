...
class EscalationManager:
    ...
    def check_escalation(self, request: ApprovalRequest) -> EscalationEvent | None:
        ...
        for rule in sorted_rules:
            ...
            if rule.trigger == EscalationTrigger.TIME_EXCEEDED:
                ...
                any_response = any(s.approver is not None for s in request.steps)
    ...
    def _create_escalation(
        ...
        logger.info(
            f"Escalated request {request.id} to {escalate_to} (rule: {rule.id})"
        )
    ...
    def escalate_manually(
        self, request: ApprovalRequest, escalate_to: str, reason: str
    ) -> EscalationEvent:
    ...
    def get_rule(self, rule_id: str) -> EscalationRule | None:
