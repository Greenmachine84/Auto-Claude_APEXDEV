...
class TokenLimit:
    ...
    def can_fit(self, usage: TokenUsage) -> bool:
        ...
        return (
            usage.prompt_tokens <= self.max_input_tokens and
            usage.completion_tokens <= self.max_output_tokens and
...
class TokenBudget:
    ...
    def can_spend(self, estimated_tokens: int, estimated_cost: float) -> bool:
        ...
        return (
            self.current_tokens + estimated_tokens <= self.max_tokens and
            self.current_cost + estimated_cost <= self.max_cost
        )
    ...
    def utilization_percent(self) -> float:
        ...
        token_util = (self.current_tokens / self.max_tokens) * 100 if self.max_tokens > 0 else 0
        cost_util = (self.current_cost / self.max_cost) * 100 if self.max_cost > 0 else 0
