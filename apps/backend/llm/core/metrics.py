...
from ..types import LLMUsage, ProviderType
...
class LLMMetrics:
    ...
    def record_success(
        self, provider: ProviderType, latency_ms: float, usage: LLMUsage
    ) -> None:
        with self._lock:
    ...
    def summary(self) -> dict[str, Any]:
        with self._lock:
            ...
            return {
                ...
                "total_requests": sum(
                    m.total_requests for m in self._provider_metrics.values()
                ),
                "total_tokens": sum(
