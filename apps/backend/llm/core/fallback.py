...
from ..types import FallbackConfig, FallbackTrigger, LLMResponse, ProviderType
...
class FallbackHandler:
    ...
    async def execute(self, request: "ChatRequest") -> LLMResponse:
        ...
        for provider_type in chain:
            ...
            try:
                ...
                logger.info(
                    "Fallback succeeded with %s after %d attempts",
                    provider_type.value,
                    result.attempts,
                )
                return response
                ...
                self._failure_counts[provider_type] = (
                    self._failure_counts.get(provider_type, 0) + 1
                )
    ...
    def _get_fallback_chain(
        self, failed_provider: ProviderType | None
    ) -> list[ProviderType]:
