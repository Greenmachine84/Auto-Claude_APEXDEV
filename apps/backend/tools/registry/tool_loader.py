...
from ..models import ParameterType, Tool, ToolCategory, ToolParameter
...
class ToolLoader:
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module):
                if isinstance(obj, Tool):
                    ...
                    if not valid:
                        logger.error("Invalid tool %s: %s", tool.name, errors)
                        continue
    ...
    async def reload_module(self, module_path: str) -> list[str]:
        ...
        for name, reg in list(self.registry._tools.items()):
            if (
                hasattr(reg.tool, "_source_module")
                and reg.tool._source_module == module_name
            ):
        ...
        for name, param in sig.parameters.items():
            ...
            parameters.append(
                ToolParameter(
                    name=name,
                    type=param_type,
                    description=tool_def.get("params", {})
                    .get(name, {})
                    .get("description", f"Parameter {name}"),
