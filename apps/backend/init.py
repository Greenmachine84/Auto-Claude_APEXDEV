...
Handles first-time setup of .apexdev directory and ensures proper gitignore configuration.
"""

from pathlib import Path


def ensure_gitignore_entry(project_dir: Path, entry: str = ".apexdev/") -> bool:
    """
    ...
    Args:
        ...
        entry: The gitignore entry to add (default: ".apexdev/")
    ...
    if gitignore_path.exists():
        ...
        for line in lines:
            ...
            # Match both ".apexdev" and ".apexdev/"
            if (
                line_stripped == entry
...
def init_auto_claude_dir(project_dir: Path) -> tuple[Path, bool]:
    """
    Initialize the .apexdev directory for a project.
    ...
    auto_claude_dir = project_dir / ".apexdev"

    # Create the directory if it doesn't exist
    dir_created = not auto_claude_dir.exists()
    auto_claude_dir.mkdir(parents=True, exist_ok=True)

    # Ensure .apexdev is in .gitignore (only on first creation)
    gitignore_updated = False
    if dir_created:
        gitignore_updated = ensure_gitignore_entry(project_dir, ".apexdev/")
    else:
...
def get_auto_claude_dir(project_dir: Path, ensure_exists: bool = True) -> Path:
    """
    Get the .apexdev directory path, optionally ensuring it exists.
