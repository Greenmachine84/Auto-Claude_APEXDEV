...
class MetricsAggregator:
    ...
    async def aggregate_by_time(
        ...
        if granularity == "hour":
            for i in range(min(period_days * 24, self.config.dashboard.time_series_points)):
    ...
    async def get_summary(
        ...
        latencies = [p.avg_latency_ms for p in by_provider.values() if p.avg_latency_ms > 0]
