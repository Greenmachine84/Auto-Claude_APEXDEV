...
from ..models import SUPPORTED_PROVIDERS, TimeSeriesPoint
...
class TimeSeriesManager:
    ...
    def __init__(self) -> None:
        ...
        logger.info(
            "TimeSeriesManager initialized with %d providers", len(SUPPORTED_PROVIDERS)
        )
    ...
    def cleanup_old_data(self, max_age_days: int = 90) -> int:
        ...
        for series_name in list(self._series.keys()):
            ...
            old_keys = [
                key for key, bucket in series.items() if bucket.timestamp < cutoff
            ]
            for key in old_keys:
                del series[key]
                removed += 1
