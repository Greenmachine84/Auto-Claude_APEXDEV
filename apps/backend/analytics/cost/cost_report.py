...
from ..models import SUPPORTED_PROVIDERS, CostRecord
...
class CostReportGenerator:
            ...
            if provider in provider_data:
                ...
                stats.append(
                    ProviderStats(
                        ...
                        average_cost_per_request=pd["cost"] / pd["count"]
                        if pd["count"] > 0
                        else 0,
                        percentage_of_total=(pd["cost"] / total_cost * 100)
                        if total_cost > 0
                        else 0,
    ...
    def _generate_recommendations(
        ...
        if len(trends) >= 7:
            ...
            earlier_avg = (
                sum(t.cost for t in trends[:7]) / 7 if len(trends) >= 14 else recent_avg
            )
