...
class ProviderComparison:
    ...
    async def compare_providers(
        ...
        max_requests = max(
            (pd["request_count"] for pd in provider_data.values()), default=1
        )
        ...
        for provider in SUPPORTED_PROVIDERS:
            ...
            # Throughput score
            throughput_score = (
                100 * pd["request_count"] / max_requests if max_requests > 0 else 0
            )
        ...
        best_by_dim = {
            ...
            "latency": max(scores, key=lambda x: x.latency_score).provider
            if scores
            else "",
            "quality": max(scores, key=lambda x: x.quality_score).provider
            if scores
            else "",
            "reliability": max(scores, key=lambda x: x.reliability_score).provider
            if scores
            else "",
            "throughput": max(scores, key=lambda x: x.throughput_score).provider
            if scores
            else "",
        }
    ...
    def _generate_recommendations(
        ...
        local_usage = sum(
            s.throughput_score for s in scores if s.provider in local_providers
        )
    ...
    async def get_cost_efficiency(
        ...
        for r in records:
            ...
            provider_efficiency[r.provider]["tokens"] += (
