...
class UsageOptimizer:
    ...
    def __init__(
        ...
        self._model_tiers = {
            "premium": ["gpt-4", "gpt-4-turbo", "claude-3-opus", "gemini-1.5-pro"],
            "standard": [
                "gpt-4o",
                "claude-3-sonnet",
                "claude-3.5-sonnet",
    ...
    async def analyze(
        ...
        # Get usage data
        summary = await self.cost_tracker.get_user_cost(user_id, start_time=start_time)
    ...
    def _analyze_local_provider_usage(
        ...
        local_cost = sum(summary.by_provider.get(p, 0.0) for p in local_providers)
    ...
    def _analyze_usage_patterns(
        ...
        off_hours_requests = [
            r for r in records if 0 <= datetime.fromisoformat(r.timestamp).hour < 6
        ]
    ...
    async def estimate_potential_savings(
        ...
        return {
            ...
            "savings_percentage": (total_savings / summary.total_cost * 100)
            if summary.total_cost > 0
            else 0,
