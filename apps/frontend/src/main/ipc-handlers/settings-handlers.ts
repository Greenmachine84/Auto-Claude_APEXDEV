...
/**
 * Auto-detect the APEXDEV source path relative to the app location.
...
const detectAutoBuildSourcePath = (): string | null => {
  ...
  for (const p of possiblePaths) {
    ...
    // This prevents matching legacy 'APEXDEV/' directories that don't have the runners
  ...
  console.warn('[detectAutoBuildSourcePath] Could not auto-detect APEXDEV source path. Please configure manually in settings.');
  ...
  ipcMain.handle(
    ...
    async (_, config: { claudeOAuthToken?: string }): Promise<IPCResult> => {
      try {
        ...
        // Generate content
        const lines: string[] = [
          '# APEXDEV Framework Environment Variables',
