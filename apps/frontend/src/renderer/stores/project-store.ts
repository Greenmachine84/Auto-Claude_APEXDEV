...
/**
 * Check APEXDEV version status for a project
 */
...
/**
 * Initialize APEXDEV in a project
 */
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      if (result.data.success) {
        console.log('[ProjectStore] Updating project autoBuildPath to .APEXDEV');
        store.updateProject(projectId, { autoBuildPath: '.APEXDEV' });
      } else {
